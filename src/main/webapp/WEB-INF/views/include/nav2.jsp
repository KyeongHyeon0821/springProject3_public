...
<body>
  <nav>
    <div class="nav-container">
      <div class="nav-left">
        <a href="${ctp}/" class="logo"><img src="${ctp}/images/logo.png" alt="로고" /></a>

        <div class="search-result" id="search-result">
          <div><img src="${ctp}/images/search.png" width="19px" class="me-2" />${searchString}</div>
          <div>${fn:replace(fn:substring(checkinDate, 5, 10), '-', '.')}</div>
          <div>-</div>
          <div>${fn:replace(fn:substring(checkoutDate, 5, 10), '-', '.')}</div>
          <div>인원 ${guestCount}</div>
          <div>반려견 ${petCount}</div>
        </div>
      </div>
